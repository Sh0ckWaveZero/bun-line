...
const handleLogin = async (req: NextApiRequest, message: string) => {
  ...
  const userPermission: any = await prisma.user.findFirst({
    where: {
      userId: userId,
    },
  });
  if (
    !userPermission ||
    !utils.compareDate(userPermission.expiresIn, new Date().toISOString())
  ) {
...
const sendMessage = (req: any, payload: any) => {
  try {
    const lineChannelAccessToken = process.env.LINE_CHANNEL_ACCESS;
    const lineHeader = {
